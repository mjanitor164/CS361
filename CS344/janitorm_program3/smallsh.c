...
void forkProcess();
void executeCommand();
void catchSIGINT(int signo); // param: signal number
void catchSIGTSTP();
...
void main(){
	...
	// keep the program running until this variable is changed
	while (keepRunning == 1){
...
void isProcessRunning(){
	...
	// loop through all processes in process array
	for (i = 0; i < numProcesses; i++)	{
		// if waiting
		// Note to self: WNOHANG flag specifies that waitpid should return immediately 
